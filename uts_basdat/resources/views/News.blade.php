...
<body style="margin-top: 0px;">
        ...
        <div class="container"><a class="navbar-brand d-flex align-items-center" href="/"><span class="bs-icon-sm bs-icon-circle bs-icon-primary shadow d-flex justify-content-center align-items-center me-2 bs-icon" style="background: rgb(55, 99, 244);"><i class="fa fa-soccer-ball-o" style="font-size: 14px;"></i></span><span>Bola Talent Hub</span></a><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            <div class="collapse navbar-collapse" id="navcol-1">
                <ul class="navbar-nav mx-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="News1">News</a></li>
    ...
    <section class="py-5">
        <div class="container py-5" style="padding-top: 0px;margin-top: -30px;">
          <div class="p-6 m-20 bg-white  shadow">
            <div class="row mb-5">
                  <div class="col text-center">
                      <h2 class="fw-bold">Breaking Ball News</h2>
                  </div><div class="newsletter8 py-5">
                <div class="container">
                <div class="row">
                    @for ($key = count($beritas) - 1; $key >= 0; $key--)
                        @if ($key % 2 == 0)
                            <!-- Kolom pertama -->
                            <div class="col-lg-6">
                                <div class="row blog-row mt-4 mb-3" id="Berita" data-toggle="modal" data-target="#newsDetails{{ $beritas[$key]->ID_Berita }}">
                                    <div class="col-md-4">
                                        <a href="#"><img src="{{ asset('news/' . $beritas[$key]->gambar_berita) }}" alt="wrapkit" class="img-fluid" /></a>
                                    </div>
                                    <div class="col-md-8">
                                        <h5><a href="#" class="text-decoration-none">{{ $beritas[$key]->Judul }}</a></h5>
                                        <p>by <strong style="color: black;">{{ $beritas[$key]->penulis }}</strong> / {{ $beritas[$key]->created_at->format('d F Y') }}</p>
                                    </div>
                                </div>
                            </div>
                        @else
                        ...
                        <!-- Modal News for each entry -->
                        <div class="modal fade" id="newsDetails{{ $beritas[$key]->ID_Berita }}" tabindex="-1" role="dialog" aria-labelledby="newsModalLabel{{ $beritas[$key]->ID_Berita }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 100%; width: 100%;">
                                <!-- Modal content -->
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <div style="text-align: center; width: 100%;">
                                            <h4 class="modal-title font-weight-bold" id="news{{ $beritas[$key]->ID_Berita }}" style="color: black;">{{ $beritas[$key]->Judul }}</h4>
                                        </div>
                                    ...
                                    <div class="modal-body" id="getNewsDetails{{ $beritas[$key]->ID_Berita }}">
                                        @php
                                            $newsDetail = DB::table('news')
                                                ...
                                                ->where('ID_Berita', $beritas[$key]->ID_Berita)
                                                ->first();
                                        @endphp
                                        ...
                                        @if ($newsDetail)
                                            ...
                                            <div>
                                                ...
                                                @endphp
                                                <p style="text-align: center; color: black;">{{ $tanggal }}</p>
                                                <div style="text-align: justify; color: black;">
                                                    <p><strong style="font-weight: bold;">{{ $newsDetail->penulis }}</strong> - {{ $newsDetail->isi_berita }}</p>
                                                </div>
                                            </div>
                                            <!-- ... -->
                                        @else
                ...
                <!-- <div class="row">
                    @for ($key = count($beritas) - 1; $key >= 0; $key--)
                        @if ($key % 2 == 0)
                            <div class="col-lg-6">
                                <div class="row blog-row mt-4 mb-3" id="Berita">
                                    <div class="col-md-4">
                                        <a href="#"><img src="{{ asset('news/' . $beritas[$key]->gambar_berita) }}" alt="wrapkit" class="img-fluid" /></a>
                                    </div>
                                    <div class="col-md-8">
                                        <h5><a href="#" class="text-decoration-none">{{ $beritas[$key]->Judul }}</a></h5>
                                        <p>by <a href="#" class="text-decoration-none">{{ $beritas[$key]->penulis }}</a> / {{ $beritas[$key]->created_at->format('d F Y') }}</p>
                                    </div>
                                </div>
                            </div>
                        @else
